...
if __name__ == "__main__":
   ...
   if verbose:
      print "analyzing period %s - %s" % (start_datetime, stop_datetime)

   # 2. divide in chapters if length > 60mins
...
FILE "%s" FLAC\n""" % os.path.basename(filename)
   ...
   # 6. Encoding aac
      ...
      enc_proc = subprocess.check_call(["/usr/bin/lame", "-m", "j", "-q", "0", "-V", "2", "-b", "112", "-B", "256", "-h", "-", "%s/%s" % (tempdir, podfilename)], stdin=cat_proc.stdout)

   #id3 = MP3("%s/%s" % (tempdir, podfilename))
   #id3["?nam"] = "Podcast - "
